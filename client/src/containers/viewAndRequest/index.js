...
class ViewAndRequest extends Component {
    ...
    render() {
        ...
        const { quantity, inCartRequests, completeRequest } = this.props.inventory;
        let error = null;
        ...
        const requestList = inCartRequests.map(function (request) {
            return (
                <Table.Row key={request.id}>
                    <Table.Cell>{request.sku}</Table.Cell>
                    <Table.Cell >{request.price}</Table.Cell>
                    <Table.Cell >{request.stock}</Table.Cell>
                    <Table.Cell >
                        <Button onClick={this.onDelete.bind(this, inventory)}>Delete</Button>
                    </Table.Cell>
                </Table.Row>
            )
        }, this);
        let tableView = <h4>No Inventories Found. Please Add Some </h4>
        let requestView = <h4>No Inventories Found. Please Add Some </h4>
